...
mod integration_with_runner {
    ...
    fn test_iterator_produces_valid_output_for_runner() {
        ...
        for part in parts {
            ...
            assert!(num >= 1 && num <= 10, "Number should be in range [1,10]");
        }
    }
    ...
    fn test_streaming_to_simulated_runner() {
        ...
        assert!(!lines_received.is_empty(), "Should receive at least one chunk");
        
        // First chunk should be the capturing group value
        let first_chunk = &lines_received[0];
        assert!(first_chunk.trim().parse::<u64>().is_ok(), "First chunk should be a number");
    }
}
